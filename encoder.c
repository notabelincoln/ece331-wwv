...
// Encodes the given wwv signal
int encode_wwv(int pin, int input)
{
	// Determines how long to generate the tone
	float threshold;
	gpio_export(pin);
	...
	for (int i = 0; i < (int)(100 * threshold); i++) {
			gpio_value(pin, 1);
			sleep(0.005);
			gpio_value(pin, 0);
			sleep(0.005);
